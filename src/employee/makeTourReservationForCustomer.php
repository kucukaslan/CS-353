...
$result_tour_section_list = $db->query($sql);
...
<html>
<head>
    <title>make a tour reservation for a customer</title>
    <meta charset="utf-8">
...
<h3>make a tour reservation for a customer <?php ?>  </h3>
...
<div style="border-radius: 5px;  background-color: #f2f2f2;  padding: 20px;">
  ...
  <label for="type">customer id:</label>
    ...
    <label for="type">tour section id:</label>
    <select name="tour_section_id" id="tour_section_id">
    <?php while($row = $result_tour_section_list->fetch_assoc())
    {
        $x = $row["ts_id"];
        $y = $row["t_id"];
        echo "<option value=\"$x\">tour section id: $x tour id: $y</option>";
    }
         ?>
    </select>

    <label for="fname">number of people</label>
    <input type="text" id="number" name="number" placeholder="number .." required="true">
